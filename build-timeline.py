...
File 3 — filename: .github/workflows/build.yml

(Type the name exactly including the dots and slashes — GitHub makes the folders as you type.)

name: Build timeline

on:
  schedule:
    - cron: "17 * * * *"        # runs hourly (at :17 past the hour)
  workflow_dispatch: {}          # lets you run it by hand from the Actions tab
  push:
    paths:                       # also rebuild when you edit the feed list/script
      - feeds.txt
      - build-timeline.py

permissions:
  contents: write                # allow the job to commit timeline.json back
